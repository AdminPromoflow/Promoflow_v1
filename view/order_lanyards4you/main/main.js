class ControllerOrdersLanyards4You {
  ...
  onSaveClick(btn) {
    // Limita el alcance a la orden (Nivel 1) donde se dio clic
    const root = btn.closest('.inner-accordion.accordion-level-1');
    if (!root) {
      alert('No se encontró el contenedor de la orden.');
      return;
    }

    const data = this.#collectJSONFromOrder(root);

    try {
      console.log(JSON.stringify(data, null, 2));
    } catch {
      alert('No se pudo serializar el JSON.');
    }
  }

  // Recolecta solo los inputs dentro de esa orden
  #collectJSONFromOrder(root) {
    const out = { order: {}, user: {}, jobs: [] };

    const ensureJob = (j) => {
      out.jobs[j] = out.jobs[j] || { job: {} };
      return out.jobs[j].job;
    };
    const ensureArrayIdx = (arr, idx) => {
      if (!Array.isArray(arr)) arr = [];
      if (!arr[idx]) arr[idx] = {};
      return arr;
    };

    // Acumula las partes de "description" por cada job SIN guardar nada en el objeto job
    const descByJob = new Map(); // jIndex -> objeto description

    const inputs = root.querySelectorAll('input[data-scope]');
    inputs.forEach((el) => {
      const ds = el.dataset;
      const val = this.#coerce(el.value ?? '');

      switch (ds.scope) {
        case 'order':
          out.order[ds.field] = val;
          break;

        case 'user':
          out.user[ds.field] = val;
          break;

        case 'address': {
          const aidx = Number(ds.aindex);
          out.user.addresses = out.user.addresses || [];
          out.user.addresses = ensureArrayIdx(out.user.addresses, aidx);
          out.user.addresses[aidx][ds.field] = val;
          break;
        }

        case 'job': {
          const jidx = Number(ds.jindex);
          const job = ensureJob(jidx);
          job[ds.field] = val;
          break;
        }
        ...
        case 'image': {
          ...
          const job = ensureJob(jidx);
          job.image = job.image || [];
          job.image = ensureArrayIdx(job.image, iidx);
        ...
        case 'text': {
          ...
          const job = ensureJob(jidx);
          job.text = job.text || [];
          job.text = ensureArrayIdx(job.text, tidx);
          job.text[tidx][ds.field] = val;
          break;
        }

        case 'desc': {
          // Construir un objeto JSON de descripción por jIndex, sin cachearlo en el job
          const jidx = Number(ds.jindex);
          const comp = ds.comp;
          const field = ds.field;

          if (!descByJob.has(jidx)) descByJob.set(jidx, {});
          const d = descByJob.get(jidx);
          d[comp] = d[comp] || {};
          d[comp][field] = val;
          break;
        }
    ...
    // Materializar "description" por cada job
    descByJob.forEach((obj, jidx) => {
      const job = ensureJob(jidx);
      try {
        job.description = JSON.stringify(obj);
      } catch {
        job.description = '';
      }
    });
    ...
    return `
      <div class="inner-accordion accordion-level-1">
        ...
        <div class="inner-accordion_content content-level-1" hidden>
          ...
          <!-- 🔘 Action bar (Nivel 1) -->
          <div class="action_bar action-bar-l1">
            <button class="btn btn-send" data-action="send" data-oindex="${this.#esc(oIndex)}">Send</button>
            <button class="btn btn-save" data-action="save" data-oindex="${this.#esc(oIndex)}">Save</button>
            <button class="btn btn-artwork" data-action="request-artwork" data-oindex="${this.#esc(oIndex)}">Request artwork</button>
          </div>
        </div>
      </div>
    `;
